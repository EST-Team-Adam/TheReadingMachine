...
# Kalman Filter #
#y <- non_zero     # take just non-zero 
y <- df_original$Compound#[!(df_original$Compound==0)]   # take the whole dataset
...
# DataFrame building #
date1 <- as.Date("2010-06-01")  #2010-06-01
date2 <- as.Date("2016-05-25")
...
legend('topright',c("Filtered Sentiment","Wheat Price","Filtered Sentiment LM","Wheat Price LM"),col=c('red','black'), lty = c(1,1,2,2))

#plot(df$Wheat, df$Filtered)
#abline(lm(df$Filtered ~ df$Wheat),col="black",lty = c(2))

#plot(lm(df$Filtered ~ df$Wheat))
