...
NEGATE = ["aint", "arent", "cannot", "cant", "couldnt", "darent",
...
BOOSTER_DICT = {"absolutely": B_INCR,
                "amazingly": B_INCR,
                "awfully": B_INCR,
                "completely": B_INCR,
                "considerably": B_INCR,
...
SPECIAL_CASE_IDIOMS = {"the shit": 3, "the bomb": 3, "bad ass": 1.5,
                       "yeah right": -2, "cut the mustard": 2,
...
class SentimentIntensityAnalyzer(object):
        ...
        if item_lowercase in self.lexicon:
            ...
            for start_i in range(0, 3):
                if (i > start_i
                    ...
                    # and emoticons (excluding the ones that
                    # immediately preceed the item) based on their
                    # distance from the current item.
                    s = scalar_inc_dec(
                        words_and_emoticons[i - (start_i + 1)], valence,
                        is_cap_diff)
    ...
    def _least_check(self, valence, words_and_emoticons, i):
        ...
        if i > 1 and words_and_emoticons[i - 1].lower() not in self.lexicon \
           and words_and_emoticons[i - 1].lower() == "least":
            if (words_and_emoticons[i - 2].lower() != "at"
        ...
        if start_i == 2:
            if words_and_emoticons[i - 3] == "never" and \
               (words_and_emoticons[i - 2] == "so"
                or words_and_emoticons[i - 2] == "this") or \
    ...
    def _amplify_qm(self, text):
        ...
        if qm_count > 1:
            if qm_count <= 3:
                # (empirically derived mean sentiment intensity rating
...
def VADER2(sentence):
    '''Function to analyse the sentiment of a single sentence
...
def article_sentiment_scoring(articles, article_col, id_col, date_col, method,
    ...
    for article in articles:
        score = dict()
        if 'VADER' in method:
            article_score = VADER2(article[article_col])
