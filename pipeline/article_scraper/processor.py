...
for spider in spiders:
    json_file = data_dir + \
        '/blog_articles_{0}_{1}.jsonl'.format(
            time.strftime("%d_%m_%Y"), spider)
    if os.path.isfile(json_file):
        flattened_article_df = flattened_article_df.append(
            pd.read_json(json_file, lines=True), ignore_index=True)
flattened_article_df.date = flattened_article_df.date.apply(
